...
@interface YHSDropDownMenuCell ()

@end

@implementation YHSDropDownMenuCell
...
- (void)setMenuModel:(id)menuModel {
    ...
    YHSDropDownMenuModel *realMenuModel = (YHSDropDownMenuModel *)menuModel;
    ...
    if (realMenuModel.menuItemIconName.length) {
        ...
        YHSDEBUGLog(@"您传入的图片为空图片,框架内部默认不做任何处理。若您的确不想传入图片，则请忽略此处打印");
    }
...
- (void)setSeparaterLineColor:(UIColor *)separaterLineColor
{
    _separaterLineColor = separaterLineColor;
    self.separaterView.backgroundColor = separaterLineColor;
}
